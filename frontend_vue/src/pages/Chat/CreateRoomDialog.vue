      ...
      q-stepper(
            ...
            .row.q-col-gutter-y-md
              .col-xs-12
                q-input(
                  ...
                  type="text" :minlength="5" :maxlength="30"
        ...
        q-step(
          ...
          .row.q-col-gutter-y-md
            .col-xs-12
              q-btn-toggle(
                spread no-caps v-model="newChatRoom.isPublic" push toggle-color="medium-sea"
                :options=`[
                  { value: false, slot: 'private' },
                  { value: true, slot: 'public' }
                ]`
              )
                template(#private)
              ...
              q-btn-toggle(
                ...
                template(#group)
                  .no-wrap.q-my-md
                    ...
                    .text-center.text-caption Todos os participantes podem visualizar e enviar mensagens. Excções podem ser aplicadas com banimento.
        ...
        q-step(
          ...
          .row.items-center
            ...
            .col-xs-4.text-right
              q-toggle(
                :modelValue="newChatRoom.maxParticipants >= 2"
                @update:modelValue="newChatRoom.maxParticipants = newChatRoom.maxParticipants >= 2 ? 0 : 100"
              )

            template(v-if="newChatRoom.maxParticipants >= 2")
              .col-xs-8
                | Quantidade máxima de pessoas na sala

              .col-xs-4
                q-input(
                  v-model="newChatRoom.maxParticipants" dense :debounce="1000"
                  type="number"
                  outlined hide-bottom-space
                )
            ...
            .col-xs-4.text-right
              q-toggle(v-model="newChatRoom.isEncrypted" color="sea")
            ...
            q-card
              q-card-section
                .column.items-center
                  ...
                  .q-mt-md(style="max-width: 240px")
                    .flex
                      .flex.full-width.justify-between
                        div Acesso:
                        .text-weight-bold {{ newChatRoom.isPublic ? 'público' : 'restrito' }}
                      .flex.full-width.justify-between
                        div Tipo:
                        .text-weight-bold {{ newChatRoom.isAnnouncements ? 'comunicados e avisos' : 'grupo de mensagens' }}
                      .flex.full-width.justify-between
                        div Capacidade:
                        .text-weight-bold {{ newChatRoom.maxParticipants === 0 ? 'sem limite' : `${newChatRoom.maxParticipants} participantes` }}
                      .flex.full-width.justify-between
                        div Criptografia:
                        .text-weight-bold {{ newChatRoom.isEncrypted ? 'habilitada' : 'desabilitada' }}

        template(v-slot:navigation)
          q-stepper-navigation.flex.justify-between
            q-btn(flat color="grey" @click="onDialogHide" label="Cancelar")
            div
...
const createChatRoom = async () => {
  const notif = Notify.create({
    ...
    try {
      const response = await chatRoomStore.createChatRoom(newChatRoom.value);
      alert('strong type retorno');
      if (response) {
        notif({
